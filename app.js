...
function toggleAuthMode() {
    ...
    const nicknameContainer = document.getElementById('nickname-container'); // NEW
    ...
    if (isLoginMode) {
        ...
        if(nicknameContainer) nicknameContainer.style.display = 'none'; // Hide on login
    } else {
        ...
        if(nicknameContainer) nicknameContainer.style.display = 'block'; // Show on signup
    }
}

// [UPGRADED] Handles Nickname Metadata Injection
async function handleAuthSubmit() {
    ...
    if (isLoginMode) {
        ...
        const res = await myDatabase.auth.signUp({ 
            email, 
            password,
            options: { data: { nickname: nickname } } // The Metadata Injection
        });
        error = res.error; data = res.data;
    }
...
// Global Helper to get Display Name safely
...
function showPage(page) {
    ...
    `;
        ...
        // [UPGRADED] Command Center UI & Tabs
        view.innerHTML = `
            <style>
...
async function updateUserNickname() {
    ...
    if (error) alert("Error updating nickname: " + error.message);
        ...
        currentUser = data.user; // Update local state immediately
        showPage('profile'); // Refresh the view to show the new name in the header
    }
}
...
async function saveMealPlan() {
    if(await isUserBanned()) return; // SOFT BLOCK CHECK
...
// [NEW] Universal Admin Delete on Frontend
async function adminDeleteContent(table, id) {
    ...
    if (error) alert("Error: " + error.message);
        ...
        // We just reload the current view to refresh the cache. For simplicity, just reload the app state.
        location.reload(); 
    }
}
...
async function loadComments(recipeId) {
    ...
    const renderList = (data) => {
        ...
        data.forEach(comment => {
            ...
            // [NEW] Interactive Admin Nicknames & Ghost Deletes
            ...
            let adminControls = '';
            if (isAdmin) {
                // Admins get clickable names to trigger the Moderation Context Menu
...
// [NEW] The Moderation Context Menu (User Management)
...
async function postComment(recipeId) {
    ...
    const { error } = await myDatabase.from('comments').insert([{
        recipe_id: recipeId,
        user_id: currentUser.id, // NEW: Capture ID for moderation
        email: currentUser.email,
        nickname: getUserDisplayName(), // Capture current nickname
        comment_text: text
    }]);
...
function switchAdminTab(tab) {
    ['inbox', 'review', 'photo', 'library', 'settings', 'family', 'broadcasts', 'campfire', 'jail', 'audit', 'users'].forEach(t => {
        ...
        if (btn) {
            // Keep the red/purple styling for the special buttons
    ...
    if (tab === 'inbox') {
        ...
        // [NEW] The Holding Cell (Warden's Ledger)
        area.innerHTML = `
        ...
        // [NEW] The Security Camera (Audit Logs)
        area.innerHTML = `
        ...
        // [NEW] User Directory (Mirror Table Search)
        area.innerHTML = `
            <div class="window-box" style="width: 100%; box-sizing: border-box;">
...
async function loadHoldingCell() {
    ...
    // We do NOT SWR cache the jail, because we want live accurate data when doing admin work.
...
async function loadAuditLogs() {
    ...
    // SWR Cache for speed, but highly responsive
    await fetchWithSWR(
        'cache_audit_logs',
        async () => {
